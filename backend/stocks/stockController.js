...
// Function to update an Indian stock
const updateIndianStock = async (req, res) => {
  try {
    const { id } = req.params;
    const updatedStock = req.body;
    let stocks = await readJSONFile(indianStocksFilePath);
    
    const stockIndex = stocks.findIndex(stock => stock.id === parseInt(id));
    if (stockIndex === -1) {
      return res.status(404).send('Stock not found');
    }
    
    stocks[stockIndex] = { ...stocks[stockIndex], ...updatedStock };
    const prioritizedStocks = calculatePriorities(stocks);
    await writeJSONFile(indianStocksFilePath, prioritizedStocks);
    
    res.status(200).json(prioritizedStocks[stockIndex]);
  } catch (err) {
    console.error(err.message);
    res.status(500).send('Server Error');
  }
...
// Function to update a US stock
const updateUSStock = async (req, res) => {
  try {
    const { id } = req.params;
    const updatedStock = req.body;
    let stocks = await readJSONFile(usStocksFilePath);
